...
1. `get_valuation()` —— 估值快照；`symbol` 可省略，自动用当前会话股票。
2. `get_financials()` —— 近两年财务指标；`symbol` 可省略。
3. `generate_fundamental_report()` —— 生成报告（无参数，自动使用已落库的估值与财务数据）。

## 参数与数据约定

- `symbol` 支持 `000001` / `000001.SZ` / `SZ000001` 等写法；省略时用当前会话股票，没有当前股票则报错（会提示先提供代码）。
