...
- **分层摘要模板**:`generate_summary` 按一级大类套模板(每类一个 JSON schema,必填字段校验 + 一次重试),二级文体按需附加字段(合同加签约双方/金额/期限,新闻加来源/时间/地点,菜谱加份量/时间……);长文档(估算 token 超过单次调用预算时)自动分块 Map-Reduce,阈值语言感知(中文约 3 万字符、英文约 15 万字符);字段缺失自动重试、仍缺补默认值。
