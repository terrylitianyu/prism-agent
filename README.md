...
- **分层摘要模板**:`generate_summary` 按一级大类套模板(每类一个 JSON schema,必填字段校验 + 一次重试),二级文体按需附加字段(合同加签约双方/金额/期限,新闻加来源/时间/地点,菜谱加份量/时间……);长文档(估算 token 超过单次调用预算时)自动分块并行 Map-Reduce,阈值语言感知(中文约 3 万字符、英文约 15 万字符);字段缺失自动重试、仍缺补默认值。
- **修订与复用**:`revise_summary` 按用户要求修改现有摘要而不重新生成(省钱);上传按内容 md5 判重,同一文档重传保留全部状态、不重复分类/摘要。
- **上传支持 PDF**:入口层(agents/textcraft.read_document)用 pypdf 逐页抽取 PDF 文字层,txt/md 等仍按 UTF-8 文本读——文件格式知识收在 agent 入口层,框架内核不感知;扫描版(无文字层)PDF 需要 OCR,不在支持范围。
